...
import com.yhh.springboot_exam.config.auth.dto.OAuthAttributes;
import com.yhh.springboot_exam.config.auth.dto.SessionUser;
import com.yhh.springboot_exam.domain.user.User;
import com.yhh.springboot_exam.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest oAuth2UserRequest) throws OAuth2AuthenticationException {
        ...
        // 현재 로그인 진행중인 서비스를 구분하는 ID (만약 하나만 사용하면 불필요. 보통은 여러 소셜 로그인을 지원하기에 필요하다)
        String registrationId = oAuth2UserRequest.getClientRegistration().getRegistrationId();

        // OAuth2 로그인 진행시 키가 되는 필드 (DBMS 로 치면 PK 같은거라 생각하면 된다)
        String userNameAttributeName = oAuth2UserRequest.getClientRegistration().getProviderDetails().getUserInfoEndpoint().getUserNameAttributeName();

        // OAuth2UserService 를 통해 가져온 OAuth2User 의 속성들을 담을 객체
